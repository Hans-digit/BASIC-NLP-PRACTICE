...
class RNN_type(torch.nn.Module):
    def __init__(self, vocabulary_len, batch_size, text_size, input_size, hidden_size, num_layers, bidirectional=False):
        ...
        if bidirectional == True:
            self.Linear1 = torch.nn.Linear(in_features = hidden_size * 2, out_features = 8)
        else:
            self.Linear1 = torch.nn.Linear(in_features=hidden_size, out_features=8)
        self.Linear2 = torch.nn.Linear(in_features = text_size * 8,
                                       out_features = 8)
    ...
    def forward(self, context):
        ...
        linear_text = linear_text.view([self.batch_size, self.text_size*8])
        linear_text = self.dropout(linear_text)
